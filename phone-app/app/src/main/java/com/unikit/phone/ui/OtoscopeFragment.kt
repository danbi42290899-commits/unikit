...
/**
 * Screen 3 -- see plan #6. Live view is PhoneCameraXProvider's local
 * PreviewView (Phone is the actual camera source today, not a relay
 * round-trip -- see camera/CameraFeedProvider.kt). LEFT EAR/RIGHT EAR/
 * CAPTURE/FREEZE/START/STOP send /ws/control commands via
 * repository.sendCommand(); CAPTURE additionally surfaces the ack's
 * mediaId/reason (see server CAPTURE persistence, plan #4).
 */
class OtoscopeFragment : Fragment(R.layout.fragment_otoscope) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<Button>(R.id.buttonLeftEar).setOnClickListener {
            sendOrWarn(repository, "SET_OTOSCOPE_LEFT")
        }
        view.findViewById<Button>(R.id.buttonRightEar).setOnClickListener {
            sendOrWarn(repository, "SET_OTOSCOPE_RIGHT")
        }
